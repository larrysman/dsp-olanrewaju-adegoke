...
def custom_splitting(features: pd.DataFrame, target: pd.Series, test_size: float=0.3, random_state: int=42) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray]:
...
def preprocessing_x_train(categorical_features: List[str], continuous_features: List[str], X_train: pd.DataFrame) -> pd.DataFrame:
...
def save_load_to_parquet_x_train(dataPATH: str, X_train: pd.DataFrame) -> pd.DataFrame:
    X_train.to_parquet(dataPATH + 'X_train_df.parquet', index=False)
    X_train_df = pd.read_parquet(dataPATH + 'X_train_df.parquet')
    return X_train_df

def resetting_index_parquet_x_train(X_train: pd.DataFrame, X_train_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def scaling_continuous_features(continuous_features: List[str], X_test: pd.DataFrame) -> pd.DataFrame:
...
def preprocessing_x_test(categorical_features: List[str], continuous_features: List[str], X_test: pd.DataFrame) -> pd.DataFrame:
...
def save_load_to_parquet_x_test(dataPATH: str, X_test: pd.DataFrame) -> pd.DataFrame:
    X_test.to_parquet(dataPATH + 'X_test_df.parquet', index=False)
    X_test_df = pd.read_parquet(dataPATH + 'X_test_df.parquet')
    return X_test_df

def resetting_index_parquet_x_test(X_test: pd.DataFrame, X_test_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    X_test_df = X_test_df.reset_index(drop=True)
    X_test = X_test.reset_index(drop=True)
    return X_test,X_test_df

def x_test_dataframe_equality_check(dataPATH: str, X_test: pd.DataFrame) -> pd.DataFrame:
...
def DataFrame_Equality(dataPATH: str, X_train: pd.DataFrame, X_test: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    X_train = x_train_dataframe_equality_check(dataPATH, X_train)
    X_test = x_test_dataframe_equality_check(dataPATH, X_test)
    return X_train,X_test

def preprocessing(categorical_features: List[str], continuous_features: List[str], X_train: pd.DataFrame, X_test: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
